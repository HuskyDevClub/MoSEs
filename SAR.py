...
class SubCentroidsHead(nn.Module):
    def __init__(
        ...
        self.Q = nn.Parameter(
            torch.zeros(self.num_classes, self.num_subcentroids), requires_grad=False
        )
...
class TextEmbeddingDataset(Dataset):
    ...
    def __getitem__(self, idx):
        return torch.tensor(self.embeddings[idx], dtype=torch.float), torch.tensor(
...
class TextClassifier:
    def __init__(self, model_name, device="cuda", embedding_type="encode"):
        ...
        if embedding_type == "encode":
            ...
            )  # for bge-m3 encode, have no .to(device)
        else:
            self.model = AutoModel.from_pretrained(
                model_name
            )  # for bge-m3 last_hidden_state
    ...
    def train(
        ...
        optimizer = torch.optim.Adam(
            self.subcentroids_head.parameters(), lr=learning_rate
        )
        ...
        if resume_checkpoint and os.path.exists(resume_checkpoint):
            ckpt = torch.load(resume_checkpoint, weights_only=False)
            self.subcentroids_head.load_state_dict(ckpt["model_state_dict"])
            optimizer.load_state_dict(ckpt["optimizer_state_dict"])
            start_epoch = ckpt["epoch"] + 1
            best_val_acc = ckpt.get("best_val_acc", 0.0)
            logger.info(
                f"Resumed from checkpoint at epoch {start_epoch}, best_val_acc={best_val_acc:.4f}"
            )
        ...
        for epoch in range(start_epoch, num_epochs):
            ...
            for batch_idx, (embeddings, labels) in enumerate(
            ...
            if val_acc > best_val_acc:
                ...
                # Save prototypes
                torch.save(
                    self.subcentroids_head.state_dict(),
            ...
            # Save checkpoint for resumption after every epoch
            checkpoint_path = os.path.join(output_dir, "checkpoint_latest.pt")
            torch.save(
                {
                    "epoch": epoch,
                    "model_state_dict": self.subcentroids_head.state_dict(),
    ...
    def predict_batch(self, texts):
        ...
        embeddings_tensor = torch.tensor(np.array(embeddings), dtype=torch.float).to(
            self.device
        )
    ...
    def get_nearest_subcentroids(self, text, embedding, n=4):
        ...
        with torch.no_grad():
            ...
            masks = torch.einsum(
                "nd,kmd->nmk", embedding_tensor, self.subcentroids_head.prototypes
            )
...
def train_model(args):
    ...
    classifier = TextClassifier(
        model_name=args.model_name, device=device, embedding_type=args.embedding_type
    )

    # Prepare data
    train_data, val_data = classifier.prepare_data(args.datasets_folder)
    logger.info(
        f"Training with {len(train_data)} samples, validating with {len(val_data)} samples"
    )
...
def test_model(args):
    ...
    classifier.load_model(
        model_name=args.subcentroids_model_name, class_names_path=args.class_names_path
    )
    ...
    for dataset_name, similarity, weight in results:
        print(
            f"Dataset: {dataset_name}, Similarity: {similarity:.4f}, Weight: {weight:.4f}"
        )
...
if __name__ == "__main__":
    ...
    train_parser.add_argument(
        "--model_name", type=str, required=True, help="Path to the BGE-M3 model"
    )
    train_parser.add_argument(
        "--datasets_folder", type=str, required=True, help="Path to the datasets folder"
    )
    train_parser.add_argument(
        "--embedding_type",
        type=str,
        default="cls",
        choices=["encode", "last_hidden_state", "cls"],
    ...
    train_parser.add_argument(
        ...
        type=int,
        default=4,
        help="Number of subcentroids per class",
    )
    train_parser.add_argument(
    ...
    train_parser.add_argument(
        "--no_cuda", action="store_true", help="Do not use CUDA even if available"
    )
    train_parser.add_argument(
        "--resume_checkpoint",
        type=str,
    ...
    # Testing arguments
    test_parser = subparsers.add_parser("test", help="Test an existing model")
    ...
    test_parser.add_argument(
        "--subcentroids_model_name",
        type=str,
        required=True,
        help="Path to the trained subcentroids model",
    )
    test_parser.add_argument(
        "--class_names_path",
        type=str,
    ...
    test_parser.add_argument(
        "--input_text", type=str, required=True, help="Input text to classify"
    )
    test_parser.add_argument(
